Feature: Tracing and Spans
  ...
  Scenario: Parallel Query
    A parallel query should produce a span for the query and a span for each worker.
    The worker span has the query span as its parent and shares the same Trace ID.

    Given I am authenticated
    And parameter `max_parallel_workers_per_gather` is `1`
    When I execute: CREATE TABLE t_par AS SELECT generate_series(1, 1000) AS id
    When I execute: SELECT count(*) FROM t_par
    Then I should see a span with:
      | kind      | server         |
      | status    | unset          |
      | span_id   | *generated*    |
