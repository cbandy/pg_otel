...
pub mod ext {
    ...
    impl SpanStatusExt for SpanStatus {}
    #[allow(dead_code)]
    pub trait SpanStatusExt {
        fn ok() -> SpanStatus {
            SpanStatus {
        ...
        fn error(message: impl Into<String>) -> SpanStatus {
            SpanStatus {
...
mod tests {
    ...
    fn test_span_status() {
        ...
        assert_eq!(ok.code, SpanStatusCode::Ok as i32);
        assert_eq!(ok.message, "");

        let err = SpanStatus::error("something went wrong");
        assert_eq!(err.code, SpanStatusCode::Error as i32);
        assert_eq!(err.message, "something went wrong");

        let err_empty = SpanStatus::error("");
        assert_eq!(err_empty.code, SpanStatusCode::Error as i32);
        assert_eq!(err_empty.message, "");
    }
}
